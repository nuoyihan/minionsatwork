...
function initialize() {
    ...
    if (script.followCondition == 0) {
        ...
        for (var j=0; j < script.endFollowTriggers.length; j ++) {
            ...
            ballEndPos = followObject.getTransform().getWorldPosition();
...
function triggerStartFollow() {
    ...
    OriginPos = startFollow();
}

function triggerEndFollow() {
    startFollowTriggered = false;
    endPos = endFollow();
    throwBall(originPos, endPos);
    
    
}

function throwBall(OriginPos,EndPos) {
    if (EndPos != null) {
        x1 = originPos['x']
        y1 = originPos['y']
        z1 = originPos['z']
        ...
        z2 = EndPos['z']
        print (x2); 
        var shoot = script.targetObject.getTransform().getWorldPosition();
        print(script.followObject.getTransform().getWorldPosition());
//        shoot1 = new vec3(shoot['x'],-1*shoot['z'],shoot['y']);
//        print(shoot1);
        global.tweenManager.setEndValue(script.followObject, "ball_throw", shoot);
        global.tweenManager.startTween(script.followObject, "ball_throw", handleBasketballReset);        
        print(proxyObject.getTransform().getLocalPosition());
    }
}


//reset function
global.handleBasketballReset = function() {
  print("resetting ball position");
  script.followObject.getTransform().setWorldPosition(new vec3(0, 0, -20))
}
