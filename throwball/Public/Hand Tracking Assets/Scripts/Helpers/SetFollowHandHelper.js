...
var triggerPos = new vec3(0,0,0);
var triggerTime;
var endPos = new vec3(0,0,0);
var endTime;
...
function onUpdate() {
    ...
    if (!handController) {
        ...
        if (startAction) {
            endFollow();
            endPos= followObject.getTransform().getWorldPosition()
        ...
        if (!startAction) {
            if (startFollowDelayTimer < startFollowDelay) {
                ...
                startFollow();
                triggerPos=followObject.getTransform().getWorldPosition()
                startFollowDelayTimer = 0;
            }
        }
    }
...
function triggerEndFollow() {
    ...
    endPos = followObject.getTransform().getWorldPosition()
    endTime = getTime()
    dirVec = [triggerPos['x']-endPos['x'], triggerPos['y']-endPos['y'],triggerPos['z']-endPos['z']]
    timeInterval = endTime - triggerTime
    
    velocity = new vec3(-2*dirVec[0]/timeInterval,-9*dirVec[1]/timeInterval,10*dirVec[2]/timeInterval)
    global.ballTest.setVelocity(velocity);
////    print('endpos')
//    print(endPos)
//    dirVec = [triggerPos['x']-endPos['x'], triggerPos['y']-endPos['y'],triggerPos['z']-endPos['z']]
//    timeInterval = endTime - triggerTime
//
//    velocity = new vec3(0*dirVec[0]/timeInterval,-2*dirVec[1]/timeInterval,15*dirVec[2]/timeInterval)
//
// 
//
//    global.ballTest.setVelocity(velocity);
